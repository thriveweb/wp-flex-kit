...
*/
if (!function_exists('masonry_grid')) {
  function masonry_grid(array $args) {
    $result = new WP_Query(get_query_args($args));
    ob_start();?>
    <div <?= $args['id'] ?> class="flex masonry_grid <?= $args['classes'] ?>">
      <?php if($result->have_posts()) {
        while($result->have_posts()) {
          $result->the_post();
          global $post;
          if ($args['post_type'] == "product") {
            new component('overview_item_product');
          } else {
            new component('masonry_item', array(
              'classes' => 'grid-item',
            ));
          }
        }
